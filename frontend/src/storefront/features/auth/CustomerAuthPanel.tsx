 ...
 *
 * The ONE implementation of every sign-in flow, hosted in two places: the
 * `/login` page (`pages/LoginPage.tsx`, the fallback for direct links and
 * guard redirects) and the in-place `AuthDialog` (what every "Sign in"
 * control opens). It owns no chrome of its own beyond the optional card, so
 * either host can frame it.
 *
 ...
 *
 * Nothing here touches the router or the URL: every flow ends in
 * `onSignedIn(customer)`, and the host decides what that means (flip the
 * session, close the dialog, navigate…).
 */
...
export function CustomerAuthPanel({
  ...
  /**
   * `page` wraps the flows in the bordered `AuthCard` (they sit on the page
   * canvas); `dialog` renders them flat — the dialog panel is already the
   * surface, and a card inside a card reads as clutter.
   */
  variant?: 'page' | 'dialog'
}) {
  ...
  const body = (
    <>
      ...
      {/* One branch for both steps: `ForgotPassword` keeps the email it was
          given, so it must stay mounted from "send code" through "reset". */}
  ...
  return variant === 'page' ? <AuthCard>{body}</AuthCard> : <div>{body}</div>
}
